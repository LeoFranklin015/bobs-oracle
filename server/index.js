...
const { hardhatClient, walletClient, bubsSepoliaClient, bubsSepoliawalletClient } = require('./clients')
...
const { convertSolidityAbiToHumanReadable, externalAPICall } = require("./utils");
...
console.log(`Watching for events on contract: ${bubsOracleContract.address} using the account: ${bubsSepoliawalletClient.account.address}`);
    ...
    onLogs: async (logs) => {
        for (let i = 0; i < logs.length; i++) {
            ...
            if (log.eventName == 'Requested') {
                ...
                const data = await externalAPICall(decodedLog.args.url, decodedLog.args.path);
                console.log(`Fetching ${decodedLog.args.path} from ${decodedLog.args.url}`);
                // console.log(data);
